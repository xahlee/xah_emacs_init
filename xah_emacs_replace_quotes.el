...
(defun htmlize-elisp-ƒ-names (p1 p2)
  "Replace curly quoted “elisp function” names to HTML markup.

 2013-05-01 todo: this doc is slightly out of date. I added also elisp variables, and there's some change in code. now it prints to a temp buffer of changed items.
...
• Some functions are from 3rd party libs, and some are not bundled with GNU emacs , e.g. 「'cl」, 「'htmlize」. They may or may not be tagged depending whether they've been loaded."
  ...
  (interactive
   (cond
    ...
    (t                                  ; all other cases
     (list (point-min) (point-max) )) ) )
  (let ( inputStr resultStr (changedFWords nil) (changedVarWords nil) )
          ...
          (let ( mStr (case-fold-search nil) (ξsomeStr inputStr) )
            (with-temp-buffer
              ...
              (while (search-forward-regexp "“\\([-a-z0-9]+\\)”" (point-max) t)
                ...
                (when (and (fboundp (intern mStr)) (> (length mStr) 2))
                  (setq changedFWords (cons mStr changedFWords ) )
              ...
              (while (search-forward-regexp "“\\([-a-z0-9]+\\)”" (point-max) t)
                (setq mStr (match-string 1) )
                (when (and (boundp (intern mStr)) (> (length mStr) 2))
                  (setq changedVarWords (cons mStr changedVarWords ) )
                  (replace-match (concat "<var class=\"elisp\">" mStr "</var>") t t)
                  ) )
              (buffer-string)
              ) ))
    (if (string= inputStr resultStr)
        (progn (message "%s" "No change needed."))
      (progn
            (delete-region p1 p2)
            (insert resultStr)
        (with-output-to-temp-buffer "*changed items*"
          (mapcar (lambda (x) (princ (format "ƒ %s" x)) (princ "\n") ) (reverse changedFWords))
          (mapcar (lambda (x) (princ (format "υ %s" x)) (princ "\n") ) (reverse changedVarWords)) ) ) ) ))
